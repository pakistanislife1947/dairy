...
router.put('/employees/:id', async (req, res, next) => {
  try {
    ...
    await db.query(
      `UPDATE employees SET name=?,phone=?,address=?,designation=?,department=?,base_salary=?,join_date=? WHERE id=?`,
...
// GET /api/hr/employees/:id/advances
router.get('/employees/:id/advances', async (req, res, next) => {
  try {
    const [rows] = await db.query('SELECT * FROM advance_salary WHERE employee_id = ? ORDER BY advance_date DESC', [req.params.id]);
    res.json({ success: true, data: rows });
  } catch (err) { next(err); }
});
...
router.post('/employees/:id/advances',
  ...
  async (req, res, next) => {
    try {
      ...
      const [result] = await db.query(
        `INSERT INTO advance_salary (employee_id,amount,advance_date,notes,created_by) VALUES (?,?,?,?,?)`,
...
router.get('/payroll', async (req, res, next) => {
  try {
    ...
    let sql = `SELECT p.*, e.name AS employee_name, e.emp_code, e.designation
...
// POST /api/hr/payroll/process — uses transaction()
  ...
  async (req, res, next) => {
    try {
      ...
      const results = await db.transaction(async (conn) => {
        const [employees] = await conn.query('SELECT * FROM employees WHERE is_active = TRUE');
        const processed = [];
        ...
        for (const emp of employees) {
          ...
          if (advDeduction > 0) {
            ...
            for (const adv of advances) {
              ...
              await conn.query('UPDATE advance_salary SET recovered=?,status=? WHERE id=?', [newRec.toFixed(2), newStat, adv.id]);
              remaining -= apply;
            }
          }
          ...
          if (salCat) {
            ...
            await conn.query(
              `INSERT INTO expenses (category_id,expense_date,amount,description,reference_type,reference_id,created_by)
               VALUES (?,?,?,?,?,?,?)`,
              [salCat.id, `${yr}-${mn}-01`, netSalary.toFixed(2), `Salary: ${emp.name} (${payroll_month})`, 'payroll', pr.insertId, req.user.id]
            );
          }

          processed.push({ id: pr.insertId, employee: emp.name, net_salary: netSalary.toFixed(2) });
        }
...
router.patch('/payroll/:id/pay', async (req, res, next) => {
  try {
    await db.query(`UPDATE payroll SET status='paid', paid_at=NOW() WHERE id=?`, [req.params.id]);
...
router.get('/users', async (_req, res, next) => {
  try {
    const [rows] = await db.query('SELECT id,name,email,role,is_active,email_verified,created_at FROM users ORDER BY created_at DESC');
    res.json({ success: true, data: rows });
  } catch (err) { next(err); }
});
...
router.patch('/users/:id/toggle', async (req, res, next) => {
  try {
    await db.query('UPDATE users SET is_active = NOT is_active WHERE id = ? AND id != 1', [req.params.id]);
