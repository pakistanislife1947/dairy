...
router.post('/:id/rent-payments',
  ...
  async (req, res, next) => {
    try {
      ...
      const [result] = await db.query(
        `INSERT INTO shop_rent_payments (shop_id, paid_for, paid_date, amount, notes, recorded_by)
         VALUES (?,?,?,?,?,?)`,
        [req.params.id, paid_for, paid_date, amount, notes || null, req.user.id]
      );

      // Central ledger
      const [[cat]] = await db.query("SELECT id FROM expense_categories WHERE name = 'Rent' LIMIT 1");
      if (cat) {
        await db.query(
