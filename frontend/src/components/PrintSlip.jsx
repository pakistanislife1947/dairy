...
export function InvoicePrint({data,onClose}) {
  ...
  return (
    <>
        ...
        <div id="invoice-print-area" style={{fontFamily:'Inter,sans-serif',background:'#fff',borderRadius:12,border:'1px solid #e2e8f0',padding:'24px',maxWidth:480,margin:'0 auto'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20,paddingBottom:16,borderBottom:'2px solid #1b6ca8'}}>
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              ...
              <div><p style={{margin:0,fontWeight:800,fontSize:16,color:'#0a2540'}}>{appName}</p><p style={{margin:0,fontSize:10,color:'#94a3b8'}}>Dairy Management</p></div>
            </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16}}>
            ...
            <div style={{textAlign:'right'}}>
              <p style={{margin:0,fontSize:9,color:'#94a3b8',textTransform:'uppercase',marginBottom:4}}>Invoice Date</p>
          ...
          <table style={{width:'100%',borderCollapse:'collapse',marginBottom:16,fontSize:12}}>
            <thead><tr style={{background:'#f8fafc'}}>
              {['Description','Qty','Rate','Amount'].map(h=><th key={h} style={{textAlign:h==='Description'?'left':'right',padding:'8px 10px',color:'#64748b',fontSize:10,textTransform:'uppercase',fontWeight:600,borderBottom:'1px solid #e2e8f0'}}>{h}</th>)}
            </tr></thead>
            <tbody>
          ...
          <div style={{background:'#f8fafc',borderRadius:8,padding:'12px 16px',marginBottom:16}}>
            {parseFloat(data.discount)>0&&<div style={{display:'flex',justifyContent:'space-between',fontSize:12,color:'#dc2626',marginBottom:6}}><span>Discount</span><span style={{fontFamily:'monospace'}}>-{fmt(data.discount)}</span></div>}
            {parseFloat(data.tax_pct)>0&&<div style={{display:'flex',justifyContent:'space-between',fontSize:12,color:'#64748b',marginBottom:6}}><span>Tax ({data.tax_pct}%)</span><span style={{fontFamily:'monospace'}}>{fmt(data.tax_amount)}</span></div>}
            <div style={{display:'flex',justifyContent:'space-between',fontSize:16,fontWeight:800,color:'#1b6ca8',borderTop:'1px solid #e2e8f0',paddingTop:8}}><span>Total</span><span style={{fontFamily:'monospace'}}>{fmt(data.total_amount)}</span></div>
            {parseFloat(data.paid_amount)>0&&<div style={{display:'flex',justifyContent:'space-between',fontSize:12,color:'#16a34a',marginTop:6}}><span>Paid</span><span style={{fontFamily:'monospace'}}>{fmt(data.paid_amount)}</span></div>}
            {balance>0.01&&<div style={{display:'flex',justifyContent:'space-between',fontSize:13,fontWeight:700,color:'#dc2626',marginTop:4}}><span>Balance Due</span><span style={{fontFamily:'monospace'}}>{fmt(balance)}</span></div>}
          </div>
          <p style={{textAlign:'center',fontSize:10,color:'#cbd5e1',marginTop:16}}>Thank you · {appName} · Quantum Solution Group</p>
        </div>
      ...
      <style>{`@media print{body>*{visibility:hidden!important;}#invoice-print-area,#invoice-print-area *{visibility:visible!important;}#invoice-print-area{position:fixed!important;left:0!important;top:0!important;width:148mm!important;padding:12mm!important;margin:0!important;border:none!important;}}`}</style>
    </>
  );
}
